...
public class SchedulerController {
    ...
    @PostMapping public ResponseEntity<ScheduledTask> create(@RequestBody ScheduledTask task) { task.setId(null); return ResponseEntity.status(HttpStatus.CREATED).body(repo.save(task)); }
    @PutMapping("/{id}") public ScheduledTask update(@PathVariable UUID id, @RequestBody ScheduledTask task) { if (!repo.existsById(id)) throw new EntityNotFoundException("Not found"); task.setId(id); return repo.save(task); }
