...
export default function Layout() {
  ...
  // Global keyboard shortcut — Cmd+K / Ctrl+K opens GlobalSearch anywhere
  useEffect(() => {
    const handler = (e) => {
  ...
  return (
    <div className="flex h-screen overflow-hidden">
      ...
      {/* Fixed-position search hint — small pill bottom-right as a discoverability cue */}
      <button
        onClick={() => setSearchOpen(true)}
        className="fixed bottom-4 right-4 z-40 flex items-center gap-2 px-3 py-2 rounded-full text-xs font-semibold shadow-lg transition-all hover:scale-105"
        style={{
