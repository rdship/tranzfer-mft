  ...
  return (
    <div className="space-y-6">
      <div><h1 className="text-2xl font-bold text-gray-900">Real-Time Activity</h1>
        <p className="text-gray-500 text-sm">Live view — auto-refreshes every 5 seconds</p></div>
        ...
        <div className="card">
          ...
          <div className="space-y-2">{transfers.map((t, i) => (
            <div key={i} className="flex items-center gap-3 p-2 bg-green-50 rounded-lg text-sm">
              ...
              {t.fileSizeBytes && <span className="text-xs text-gray-400">{(t.fileSizeBytes / 1024).toFixed(0)} KB</span>}
      ...
      <div className="card">
        ...
        {events.length === 0 ? <p className="text-sm text-gray-500">No events yet. Activity will appear here as files are transferred.</p> : (
          <div className="space-y-1 max-h-96 overflow-y-auto">{events.map((e, i) => (
            <div key={i} className="flex items-center gap-2 text-xs py-1 border-b border-gray-50">
              <span className="text-gray-400 w-16 font-mono">{e.timestamp ? format(new Date(e.timestamp), 'HH:mm:ss') : ''}</span>
