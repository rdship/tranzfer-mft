...
| N33 | **SEDA pipeline disconnected — 0 transfer records created after file uploads (DEMO BLOCKER)** | CRITICAL | **OPEN** | Uploaded 38 files via 3rd-party SFTP + FTP clients to 9 accounts. SFTP service publishes `FileUploadedEvent` to RabbitMQ `file-transfer.events` exchange. Events are consumed from `sftp.account.events` queue. But `file_transfer_records` and `flow_executions` tables remain at 0 rows. Activity Monitor shows nothing. The intake→match→process→deliver pipeline never creates persistent records. **Evidence:** 10 FileUploadedEvents in sftp-service logs, 0 rows in DB, 0 Activity Monitor entries. See `demo-flow-test-report-20260416.md` for full analysis. |
...
| N37 | **Config service GET /api/flows returns 500 — FlowStep not Serializable (UI shows "0 flows configured")** | CRITICAL | **OPEN** | **Symptom:** UI "Processing Flows" page shows "0 flows configured" despite 21 flows in DB. All paths (config-service:8084, gateway:8085, onboarding:8080) return HTTP 500. **Root cause:** `FileFlowDto` implements `Serializable` but its field `List<FileFlow.FlowStep> steps` references `FileFlow.FlowStep` which does NOT implement `Serializable`. The `@Cacheable` on `getAllFlows()` in `FileFlowController` triggers JDK serialization to Redis, which fails on `FlowStep`. **Stack trace:** `NotSerializableException: com.filetransfer.shared.dto.FileFlowDto` → `JdkSerializationRedisSerializer.serialize()` → `RedisCache.put()` → `CacheInterceptor.invoke()` → `FileFlowController$$SpringCGLIB$$0.getAllFlows()`. **FLUSHALL does NOT fix** — next GET tries to PUT result back into cache and fails again. **Fix (exact):** Add `implements java.io.Serializable` to `FileFlow.FlowStep` at `shared/shared-platform/src/main/java/com/filetransfer/shared/entity/transfer/FileFlow.java:96`. **Alternative fix:** Configure `RedisCacheConfiguration` to use `GenericJackson2JsonRedisSerializer` instead of `JdkSerializationRedisSerializer`, which avoids Serializable requirement entirely. **File:** `config-service/src/main/java/com/filetransfer/config/controller/FileFlowController.java` (cache annotation), `shared/.../entity/transfer/FileFlow.java:96` (missing Serializable). **Evidence:** Screenshot attached — UI at `localhost/flows` shows "No flows configured" while `SELECT count(*) FROM file_flows` returns 21. |
