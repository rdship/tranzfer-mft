...
| N48 | **Flow executions not started — FileUploadEventConsumer bean not loading despite FLOW_RULES_ENABLED=true** | CRITICAL | **OPEN** | N33 is fixed: FileUploadedEvents flow through RabbitMQ, transfer records are created, Activity Monitor shows entries. But `flow_executions` table remains at 0 rows. The RoutingEngine creates `file_transfer_records` with status=PENDING but never starts a `FlowExecution`. Flow rule registry has 7 flows compiled (including a catch-all `.*` and a specific `.*\.850$`). Activity Monitor shows `flow=None` on all entries. **Root cause hypothesis:** The RoutingEngine may create the transfer record in the `onFileUploadedInternal()` path but the flow matching + execution creation may be in a separate Kafka Fabric stage (`flow.intake`) that isn't consuming, or the VFS storage bridge needs to persist the file before matching can proceed. **Evidence:** 2 transfer records (PENDING), 0 flow executions, Activity Monitor shows 2 entries with flow=None, 7 flows compiled in registry. |

| N50 | **VFS write handle orphaned on fast SFTP session close — file not persisted to storage-manager** | HIGH | **OPEN** | When using a VIRTUAL storage mode account, the `SftpFileSystemFactory` activates VFS (`SFTP virtual filesystem ready`). But if the SFTP client disconnects quickly after `put` (e.g., scripted `<<<` here-string), the session cleanup fires before the VFS write handle completes: `flushed 1 orphaned write handle(s)`. The file is never persisted to storage-manager (0 write_intents), no FileUploadedEvent fires, no transfer record is created. **Root cause:** VFS write is asynchronous — the file handle `close()` should synchronously flush to storage-manager before returning, so the SFTP session can't end before the write is committed. **Evidence:** `SFTP virtual filesystem ready for user=vfs-test-sftp`, then immediately `flushed 1 orphaned write handle(s)`. 0 write_intents in DB. **Fix:** Make VFS file handle `close()` synchronous — block until storage-manager confirms the write. |
