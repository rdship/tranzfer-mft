...
| N21 | **Entity package restructure causes ALL DB services to freeze at HikariPool — platform cannot boot** | CRITICAL | **INVESTIGATING** | Freeze is likely during Hibernate EntityManagerFactory creation (main thread), NOT after HikariPool. The "HikariPool - Start completed" log is from a background thread. DEBUG logging added to JAVA_TOOL_OPTIONS. **CRITICAL: Tester MUST rebuild Docker images** (`docker compose build --no-cache`) — old compiled .class files reference `com.filetransfer.shared.entity.XYZ` (flat package) which no longer exists after restructure. Stale images = ClassNotFoundException during Hibernate metamodel = silent hang. |
