...
| N37 | **Config service GET /api/flows returns 500 — FileFlowDto not Serializable** | HIGH | **OPEN** | `@Cacheable` on `getAllFlows()` tries to serialize `FileFlowDto` to Redis via JDK serialization. `FileFlowDto` doesn't implement `Serializable`. `FLUSHALL` on Redis doesn't help — next request tries to PUT back into cache and fails again. **Fix options:** (1) `FileFlowDto implements Serializable`, (2) Use Jackson JSON Redis serializer, (3) Remove `@Cacheable` from list endpoint. |
