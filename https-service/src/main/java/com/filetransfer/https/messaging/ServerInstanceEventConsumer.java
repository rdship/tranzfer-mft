...
public class ServerInstanceEventConsumer {
    ...
    void subscribeOutboxEvents() {
        if (outboxPoller == null) {
            log.info("[HTTPS][server-instance] boot — @RabbitListener only; UnifiedOutboxPoller not in context");
            return;
        }
        outboxPoller.registerHandler("server.instance.", row -> {
            log.info("[HTTPS][server-instance][outbox] row id={} routingKey={} aggregateId={}",
                    row.id(), row.routingKey(), row.aggregateId());
            @SuppressWarnings("unchecked")
            Map<String, Object> payload = row.as(Map.class, objectMapper);
            onChange(payload);
        });
        log.info("[HTTPS][server-instance] boot — dual-consume ACTIVE: @RabbitListener + outbox handler (R134S)");
    }
