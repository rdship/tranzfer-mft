...
import { getFolderMappings, createFolderMapping, deleteFolderMapping, getAccounts } from '../api/accounts'
...
export default function FolderMappings() {
  ...
  const [form, setForm] = useState({ sourceAccountId: '', sourcePath: '/inbox', destinationAccountId: '', destinationPath: '/outbox', filenamePattern: '', encryptionOption: 'NONE' })
  ...
  const createMut = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries(['folder-mappings']); setShowCreate(false); toast.success('Mapping created') },
      ...
      {mappings.length === 0 ? (
        <div className="card">
          <EmptyState title="No folder mappings" description="Create a mapping to route files between accounts. This is the core of file routing."
            action={<button className="btn-primary" onClick={() => setShowCreate(true)}><PlusIcon className="w-4 h-4" />Create Mapping</button>} />
        </div>
      ) : (
        ...
        <Modal title="Create Folder Mapping" size="lg" onClose={() => setShowCreate(false)}>
          <form onSubmit={e => { e.preventDefault(); createMut.mutate(form) }} className="space-y-4">
            ...
            <div className="flex gap-3 justify-end pt-2">
              ...
              <button type="submit" className="btn-primary" disabled={createMut.isPending}>{createMut.isPending ? 'Creating...' : 'Create Mapping'}</button>
            </div>
          </form>
        </Modal>
      )}
