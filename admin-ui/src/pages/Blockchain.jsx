  ...
  return (
    <div className="space-y-6">
      ...
      {proof && (
        <div className={`card border ${proof.verified ? 'border-green-200 bg-green-50' : 'border-red-200 bg-red-50'}`}>
          ...
          <div className="grid grid-cols-2 gap-3 text-sm">
            <div><span className="text-gray-500">Track ID:</span> <strong className="font-mono">{proof.trackId}</strong></div>
            <div><span className="text-gray-500">File:</span> <strong>{proof.filename}</strong></div>
            <div><span className="text-gray-500">Chain:</span> <strong>{proof.chain}</strong></div>
            <div><span className="text-gray-500">Anchored:</span> <strong>{proof.anchoredAt}</strong></div>
            <div className="col-span-2"><span className="text-gray-500">SHA-256:</span> <code className="text-xs font-mono break-all">{proof.sha256}</code></div>
            <div className="col-span-2"><span className="text-gray-500">Merkle Root:</span> <code className="text-xs font-mono break-all">{proof.merkleRoot}</code></div>
          </div>
      ...
      <div className="card">
        ...
        {anchors.length === 0 ? <p className="text-sm text-gray-500">No anchors yet. Transfers are anchored every hour.</p> : (
          <table className="w-full"><thead><tr className="border-b">
            ...
            {anchors.slice(0, 20).map(a => (
              <tr key={a.id} className="table-row cursor-pointer hover:bg-blue-50" onClick={() => { setVerifyId(a.trackId); verify() }}>
                ...
                <td className="table-cell text-xs text-gray-500">{a.anchoredAt ? format(new Date(a.anchoredAt), 'MMM d HH:mm') : ''}</td>
              </tr>
            ))}
          </tbody></table>
        )}
